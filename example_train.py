...
# load data path list
data_path = '.'
train_set = glob.glob(data_path + '/train/*_grad.png')
valid_set = glob.glob(data_path + '/validation/*_grad.png')
test_set = glob.glob(data_path + '/test/*_grad.png')

# set key for input images and labels
org_suffix = '_grad.png'
lab_suffix = '_lab.png'
mask_suffix = '_img.png'
...
train_provider = DataProvider(train_set, [org_suffix, lab_suffix, mask_suffix],
                        is_pre_load=False,
                        is_shuffle=True,
                        processor=processor)

validation_provider = DataProvider(valid_set, [org_suffix, lab_suffix, mask_suffix],
                        is_pre_load=False,
                        processor=processor)
...
model = SimpleTFModel(unet, org_suffix, lab_suffix, mask_suffix, dropout=0, loss_function={'cross-entropy':1.0})
...
# evaluate test data
test_provider = DataProvider(test_set, [org_suffix, lab_suffix, mask_suffix],
                        is_pre_load=False,
                        processor=processor)
